...
contract FundSixLock is Ownable, ReentrancyGuard {
    ...
    // unlock token per month, 50 % in total 6 months
    uint256 public claimPeriod = 30 * 24 * 3600; // 修改为每月（30天）
    ...
    function hasClaimableTokens(address account) external view returns (bool) {
        uint256 coe = getIDOUnlockRatio();
        if (coe == 0) return false;
        uint256 expectedAmount = getExpectedAmount(account);
        uint256 canClaimAmount = (expectedAmount * (coe)) / 10000;
        canClaimAmount = (canClaimAmount * 10**18) / joinIdoPrice;
        return canClaimAmount > _alreadyClaimNumArr[account];
    }
    ...
    function getClaimableTokens(address account) external view returns (uint256) {
        ...
        uint256 canClaimAmount = (expectedAmount * (coe)) / 10000;
        canClaimAmount = (canClaimAmount * 10**18) / joinIdoPrice;
        if (canClaimAmount > _alreadyClaimNumArr[account]) {
            return canClaimAmount - _alreadyClaimNumArr[account];
        }
        return 0;
    }
    ...
    function getIDOUnlockRatio() public view returns (uint256) {
        ...
        // unlock 50% in 6 months
        uint256 period = (block.timestamp - startTime - dt - claimDt1) / claimPeriod;
        if (period > 6) return 10000; // 修改为6个月
        uint256 unlockRatio = 5000 * period / 6; // 修改为6个月
