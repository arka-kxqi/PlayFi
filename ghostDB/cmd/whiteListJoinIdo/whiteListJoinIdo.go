...
func ListJoinIdoWithWithWhiteList(memoryCache *cache.Cache, pre, after int, contractAddress, abiFileName, mintFuncName string, amount *big.Int) {
	...
	for rows.Next() {
		...
		proof, err := GetMerkleProofForAddress(address)
		if err != nil {
		...
		// 调用合约的 joinIdo 函数
		txHash := cryptoAes.BatchMintTx(client, common.HexToAddress(address), privateKeyECDSA, contractAddress, amount.Int64(), data)
		if err != nil {
			log.Fatalf("调用合约失败: %v", err)
		}
		fmt.Println("🔨", id, "正在joinIdo的钱包地址::--", address, "成功，交易哈希:", txHash)
...
func JoinIdoNoWhite(memoryCache *cache.Cache, pre, after int, contractAddress, abiFileName, mintFuncName string, amount *big.Int) {
	db, err := sql.Open("sqlite3", "../../ghostWallets.db")
	if err != nil {
		log.Fatal(err)
	}
